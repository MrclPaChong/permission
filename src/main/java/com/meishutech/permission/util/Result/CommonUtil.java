...
public class CommonUtil {
	 ...
	 * 删除成功返回
	 */
	public static JSONObject successDelJson() {
		JSONObject resultJson = new JSONObject();
		resultJson.put("code", Constants.SUCCESS_CODE);
		resultJson.put("msg", Constants.SUCCESS_MSG_DEL);
		return resultJson;
	}
	 ...
	 * 删除失败返回
	 */
	public static JSONObject errDelJson() {
		JSONObject resultJson = new JSONObject();
		resultJson.put("code", Constants.ERR_CODE);
		resultJson.put("msg", Constants.SUCCESS_MSG_DEL_ERR);
		return resultJson;
	}
	...
	public static JSONObject successPage(final JSONObject requestJson, Set<JSONObject> list, int totalCount) {
		int pageRow = requestJson.getIntValue("pageRow");
		int totalPage = getPageCounts(pageRow, totalCount);
		JSONObject result = successJson();
		JSONObject info = new JSONObject();
	...
	public static JSONObject successPageObject(final JSONObject requestJson, List<Object> list, int totalCount) {
		int pageRow = requestJson.getIntValue("pageRow");
		int totalPage = getPageCounts(pageRow, totalCount);
		JSONObject result = successJson();
		JSONObject info = new JSONObject();
	...
	private static void fillPageParam(final JSONObject paramObject, int defaultPageRow) {
		...
		String searchA = paramObject.getString("searchA");
		paramObject.put("searchA", searchA);
		paramObject.put("delete", '1');
	 ...
	 * @throws Exception
	 */
	public static void sendJsonMessage(HttpServletResponse response, JSONObject requestJson) throws Exception {
		response.setContentType("application/json; charset=utf-8");
		PrintWriter writer = response.getWriter();
		writer.print(JSONObject.toJSONString(requestJson, SerializerFeature.WriteMapNullValue,
